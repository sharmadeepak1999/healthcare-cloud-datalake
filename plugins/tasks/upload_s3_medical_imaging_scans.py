...
def upload_s3_medical_imaging_scans(bucket_name, ti):
    ...
    #     s3_key = f"dataset/images/medical-imaging/{row['filename']}"
    #     try:
    #         s3_url = upload_to_s3(bucket_name, os.path.join(os.environ.get('AIRFLOW_HOME'), f"datasets/medical-imaging/input/images/{row['filename']}"), s3_key)
    #     except:
    #         s3_url = None
